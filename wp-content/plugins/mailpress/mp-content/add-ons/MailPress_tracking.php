...
// for reports
	...
	{
		return self::get_info('browser', $useragent);
	}

	public static function get_os($useragent)
	{
		return self::get_info('os', $useragent);
	}

	public static function get_info($info, $useragent)
	{
		$file = MP_ABSPATH . "mp-admin/xml/{$info}s.xml";
		...
		if ($x)
		{
			...
			foreach ($xml->{$info} as $i)
			{
    				if (@preg_match($i->pattern, $useragent, $regmatch))
				{
					if (!isset($i->name)) return '';
					$version = false;
					$txt = '';
					if (isset($i->icon) && !empty($i->icon)) $icon = $i->icon;
					if (isset($i->versions))
					{
						foreach($i->versions as $attrs) $vp = (int) $attrs['pattern'];

						if (isset($i->versions->id))
						{
							foreach($i->versions->id as $ver)
							{
								...
								{
									$version = $ver->name;
									if (isset($ver->icon) && !empty($ver->icon)) $icon = $ver->icon;
									break;
								}
							}
						}
						elseif (!empty($i->versions))
						{
							@preg_match($i->versions, $useragent, $regmatch);
							if (isset($regmatch[$vp])) $version = $regmatch[$vp];
						}
						else
						{
							$version =  $regmatch[$vp];
