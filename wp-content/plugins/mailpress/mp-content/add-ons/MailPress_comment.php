	...
	public static function comment_form($post_id) 
	{
		...
		if (is_email($email))
		{
			...
			if ($i)
			{
				...
				if ($is_subscriber)
				{
	           			$url = MP_Users::get_unsubscribe_url(MP_Users::get_key_by_email($email));
?>
	...
	public static function comment_post($id) 
	{
		...
		if (is_email($email))
		{
			...
			else
			{
				if (isset($_POST['MailPress']['subscribe_to_comments']))
				{
					$mp_user_id = MP_Users::insert($email, $name);
					if ($mp_user_id)
					{
						add_post_meta($post_id, self::meta_key, $mp_user_id);
						MailPress::update_stats('c', $post_id, 1);
...
////  Post ////
	...
	{
		global $wpdb;
		MailPress::update_stats('c', $id, (-1 * $wpdb->get_var("SELECT sum(scount) FROM $wpdb->mp_stats WHERE stype = 'c' AND slib = '$id';")));
	}
...
//// mp_user ////
	...
	{
		if (self::has_subscribed($mp_user_id)) MailPress::update_stats('u', 'comment', -1);
	}
	...
	{
		if (self::has_subscribed($mp_user_id)) MailPress::update_stats('u', 'comment', (-1 * self::unsubscribe($mp_user_id)));
	}
...
// for posts list
	...
	{
		//$last_key  = array_pop(array_flip($x));
		//$last_data = array_pop($x); 
		$x['mp_users']=  __('Subscribers', MP_TXTDOM);
		//$x[$last_key] = $last_data;
		return $x;
	}
