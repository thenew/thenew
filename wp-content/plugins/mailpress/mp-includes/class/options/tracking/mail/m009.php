...
class MP_Tracking_module_m009 extends MP_Tracking_module_abstract
{
	...
	function meta_box($mail)
	{
		...
		$dend   = $wpdb->get_var( $wpdb->prepare( "SELECT max(DATE(tmstp)) FROM $wpdb->mp_tracks WHERE mail_id = %d ;", $mail->id) ); $y = substr($dend,0,4); $m = substr($dend,5,2); $d = substr($dend,8,2);
		$dbeg = date('Y-m-d', mktime(0, 0, 0, $m, $d-65, $y)); 												$y = substr($dbeg,0,4);	$m = substr($dbeg,5,2); $d = substr($dbeg,8,2);

		$tracks = $wpdb->get_results( $wpdb->prepare( "SELECT DATE(tmstp) as tmstp, track, count(*) as count FROM $wpdb->mp_tracks WHERE mail_id = %d AND DATE(tmstp) BETWEEN %s AND %s GROUP BY 1, 2 ORDER BY 1 DESC, 2 DESC ;", $mail->id, $dbeg, $dend) );
		if (!$tracks) return;
