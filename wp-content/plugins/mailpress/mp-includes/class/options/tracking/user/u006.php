...
class MP_Tracking_module_u006 extends MP_Tracking_module_abstract
{
	...
	// u006_user_settings
		...
		$u['u006_user_settings'] = MP_Usermeta::get($mp_user->id, '_MailPress_' . self::prefix);
		if (!$u['u006_user_settings']) $u['u006_user_settings'] = get_user_option('_MailPress_' . self::prefix);
	...
	// u006
		...
		$tracks = $wpdb->get_results( $wpdb->prepare( "SELECT DISTINCT ip FROM $wpdb->mp_tracks WHERE user_id = %d ;", $mp_user->id) );
...
new MP_Tracking_module_u006(__('Geoip', MP_TXTDOM));