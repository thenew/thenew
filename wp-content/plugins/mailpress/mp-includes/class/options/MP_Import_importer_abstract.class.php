<?php 
abstract class MP_Import_importer_abstract
{
	const bt 			= 133;
	...
	function register($importers) 
	{
		...
		$importers[$this->importer] = array ( $this->importer, $this->description, $this->callback);
		return $importers;
	}
...
// for logs
	...
	function start_trace($step)
	{
		...
		$this->trace = new MP_Log($this->importer, MP_ABSPATH, 'MP_Import', false, 'import');
...
////  IMPORT API  ////
	...
	function sync_mp_user_newsletter($mp_user_id) 
	{
		...
		MP_Newsletters::set_object_terms( $mp_user_id, array_merge(MP_Newsletters::get_object_terms($mp_user_id), MP_Newsletters::get_defaults()) );		
	}
}