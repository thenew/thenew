<?php
abstract class MP_Forms_field_type_abstract
{
	var $field  = null;
	...
	function __construct($description)
	{
		...
		add_filter('MailPress_form_field_type_' . $this->field_type . '_have_file',		array(&$this, 'have_file'), 		8, 1);

		add_filter('MailPress_form_field_type_' . $this->field_type . '_submitted',		array(&$this, 'submitted'), 		8, 1);

		add_filter('MailPress_form_field_type_' . $this->field_type . '_get_tag',		array(&$this, 'get_tag'), 		8, 2);
		add_filter('MailPress_form_field_type_' . $this->field_type . '_get_id',		array(&$this, 'get_id'), 		8, 1);
		add_filter('MailPress_form_field_type_' . $this->field_type . '_get_name',		array(&$this, 'get_name'), 		8, 1);

		add_action('MailPress_form_field_type_' . $this->field_type . '_settings_form',	array(&$this, 'settings_form'),	8, 1);
	}

	function register($field_types) { $field_types[$this->field_type] = array( 'desc' => $this->description, 'order' => $this->order ); return $field_types; }
	...
	{
		$this->field = $field;
		$this->type_ok = ($this->field_type == $this->field->type);
...
?>
<div id='field_type_<?php echo $this->field_type; ?>_settings' class='field_type_settings'<?php echo ( ($this->type_ok) ? '' : "style='display:none;'" ); ?>>
	<ul>
<?php 	foreach ($tabs as $tab_type => $tab) echo "<li><a href='#settings_tab_" . $this->field_type . '_' . $tab_type . "'><span>$tab</span></a></li>\n"; ?>
	</ul>
	<div style='clear:both;' >
<?php
		foreach ($tabs as $tab_type => $tab) 
		{
			echo "\n<div id='settings_tab_" . $this->field_type . '_' . $tab_type . "' class='ui-tabs settings_form_tabs settings_$tab_type'>\n";
			switch ($tab_type)
			{
				case 'html' :
							...
							foreach ($child->children() as $tags)
							{
								switch ($tags->getName())
								{
									case 'checkbox' :
										...
										echo "<label for='" . $this->prefix . $this->field_type . '_settings_' . $tab_type . '_' . $attribute . '_' . $tags->value . "' class='inline' ><span class='description'><small>" . $tags->text . "</small></span></label>&nbsp;\n";
									break;
									case 'hidden' :
									...
									case 'is' :
										...
										{ 
											...
											echo "<label for='" . $this->prefix . $this->field_type . '_settings_' . $tab_type . '_' . $attribute . '_' . $attribute . "' class='inline' ><span class='description'><small>$attribute</small></span></label>&nbsp;\n";
										}
									break;
									case 'misc' :
...
<?php
	...
	function settings_hidden($setting, $attribute) { ?><input type='hidden' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' value="<?php if ($this->type_ok && isset($this->field->settings[$setting][$attribute])) echo esc_attr($this->field->settings[$setting][$attribute]); ?>" /><?php }
	function settings_hidden_value($setting, $attribute, $value) { ?><input type='hidden' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' value="<?php echo esc_attr($value); ?>" /><?php }
	function settings_text($setting, $attribute, $disabled = false, $default = '', $size = 32) { ?><input type='text' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_' . $setting . '_' . $attribute; ?>' value="<?php echo ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? esc_attr($this->field->settings[$setting][$attribute]) : esc_attr($default) ); ?>" size='<?php echo $size; ?>' /><?php }
	function settings_checkbox($setting, $attribute, $value, $disabled = false, $class = false, $forced = false) { ?><input type='checkbox' name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_' . $setting . '_' . $attribute . '_' . $value; ?>' value="<?php echo esc_attr($value); ?>" style='width:auto;'<?php if ($disabled) echo " disabled='disabled'"; ?><?php if ($class) echo " class='$class'"; ?><?php if ($forced !== false) checked(true); else checked( $value, ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? $this->field->settings[$setting][$attribute] : null ) ); ?> /><?php if ($forced !== false) return true; return ($this->type_ok && isset($this->field->settings[$setting][$attribute]) && ($value == $this->field->settings[$setting][$attribute]) );}
	function settings_radio($setting, $attribute, $value, $value_text, $default, $disabled = false)	{ ?><input type='radio'	name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type .'_settings_' . $setting . '_' . $attribute . '_' . $value; ?>' 	value="<?php echo esc_attr($value); ?>" style='width:auto;'<?php checked($value, ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? $this->field->settings[$setting][$attribute] : $default) ); ?><?php if ($disabled) echo " disabled='disabled'"; ?> /><label class='inline' for='<?php echo $this->prefix . $this->field_type .'_settings_' . $setting . '_' . $attribute . '_' . $value; ?>' ><span class='description'><small<?php if ($disabled) echo " style='color:#888'";?>><?php echo $value_text; ?></small></span></label>&nbsp;<?php }
	function settings_select_num($setting, $attribute, $min, $max, $default) { ?><select name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_' . $setting . '_' . $attribute; ?>' ><?php MP_AdminPage::select_number($min, $max,     ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? $this->field->settings[$setting][$attribute] : $default) ); ?></select><?php }
	function settings_select_opt($setting, $attribute, $values, $default) { ?><select name='<?php echo $this->prefix . $this->field_type . "[settings][$setting][$attribute]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_' . $setting . '_' . $attribute; ?>' ><?php MP_AdminPage::select_option($values, ( ($this->type_ok && isset($this->field->settings[$setting][$attribute])) ? $this->field->settings[$setting][$attribute] : $default) ); ?></select><?php }
	function settings_attributes_textarea($text, $option) { ?><input type='hidden' name='<?php echo $this->prefix . $this->field_type . '[textareas][]'; ?>' value='<?php echo esc_attr($option); ?>' /><?php $this->settings_description($text); ?><br /><textarea name='<?php echo $this->prefix . $this->field_type . "[settings][attributes][$option]"; ?>' id='<?php echo $this->prefix . $this->field_type . '_settings_attributes_' . $option; ?>' cols='40' rows='4'><?php if ($this->type_ok && isset($this->field->settings['attributes']['tag_content'])) echo esc_attr(trim(base64_decode($this->field->settings['attributes']['tag_content']))); ?></textarea><?php }
}