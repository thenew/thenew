...
class MP_Newsletters_processors extends MP_Options
{
	...
	var $abstract = 'Newsletters_processor_abstract';
	...
	public static function send($newsletter, $trace = false, $qp = true)
	{
		...
		self::message_report($newsletter, 'query_posts : ' . json_encode($newsletter['query_posts']), $trace);

		$rc = MP_Newsletters::send($newsletter, $qp, false, $trace);
		...
		if ($trace)
		{
			...
			switch ( true )
			{
				...
				break;
				default :
					$bm = "** Process successfull ** (recipients : $rc)";
				break;
			}
