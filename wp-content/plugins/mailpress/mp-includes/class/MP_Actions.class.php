...
////  VIEW MAIL/THEME in thickbox  ////
	...
	// from
		$from 	= ('sent' == $mail->status) ? MP_Mails::display_toemail($mail->fromemail, $mail->fromname) : MP_Mails::display_toemail($mp_general['fromemail'], $mp_general['fromname']);
	// to
		$to 		= MP_Mails::display_toemail($mail->toemail, $mail->toname, '', $user);
	// subject
		MailPress::require_class('Mail');
		$x = new MP_Mail();
		$subject 	= ('sent' == $mail->status) ? $mail->subject : $x->do_eval($mail->subject);
		$subject 	= $x->viewsubject($subject, $id, $main_id, $user);
	// template
		$template   = ('sent' == $mail->status) ? false : 'manual';
	// content
		$args			= array();
