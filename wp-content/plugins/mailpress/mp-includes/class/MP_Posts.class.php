...
class MP_Posts
{
	const metakey = MailPress_post::metakey;
	const metakey_order =  MailPress_post::metakey_order;
	...
	public static function insert($mp_mail_id, $post_id) 
	{
		...
		return MP_Mailmeta::add( $mp_mail_id, self::metakey, $post_id );
	}
	...
	public static function delete($mp_mail_id, $post_id) 
	{
		...
		else 				MP_Mailmeta::update( $mp_mail_id, self::metakey_order, $meta_value );

		return MP_Mailmeta::delete( $mp_mail_id, self::metakey, $post_id );
	}

	public static function delete_post($post_id) 
	{
		global $wpdb;
		$mails = $wpdb->get_results( $wpdb->prepare( "SELECT mp_mail_id FROM $wpdb->mp_mailmeta WHERE meta_key = %s AND meta_value = %s", self::metakey, $post_id ) );
...
// retourne les drafts d'un post
	...
	{
		global $wpdb;
		$mails = $wpdb->get_results( $wpdb->prepare( "SELECT mp_mail_id FROM $wpdb->mp_mailmeta WHERE meta_key = %s AND meta_value = %s ORDER BY mp_mail_id;", self::metakey, $post_id ) );
