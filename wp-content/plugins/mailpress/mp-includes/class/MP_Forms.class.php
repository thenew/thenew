...
//// the form /building/resetting/controlling 
	...
	public static function form( $id )
	{
		...
		foreach($fields as $k => $field) 
		{
			...
			$fields[$k] = ($submitted) ? MP_Forms_field_types::submitted( $field ) : $field;
	...
	public static function process( $form, $fields, $form_templates)
	{
		...
		foreach($fields as $k => $field)
		{
			...
			$incopy = (isset($field->settings['options']['incopy']) && $field->settings['options']['incopy']);
  			...
  			$map = (isset($field->submitted['map'])) ? self::handle_map($field->submitted['map']) : false;
			$img = (!$map) ? '' : "<img align='center' style='border:none;margin:0;padding:0' src='" . get_option('siteurl') . '/' . MP_PATH . $map . "' alt='' /></br>\r\n";
			...
			if ($map) $array->field[$field->id]['map']= $map;
		...
		if ('0' != $form->settings['visitor']['subscription'] && isset($visitor_toemail))
		{
				...
				switch($mp_user->status) 
				{
					...
					case 'waiting':
						switch ($form->settings['visitor']['subscription'])
						{
							...
							break;
							case '2' :	// tbc
								MP_Users::send_confirmation_subscription($mp_user->email, $mp_user->name, $mp_user->confkey);
							break;
							case '3' :	// active
								MP_Users::set_status($mp_user->id, 'active');
							break;
						}
					break;
					case 'unsubscribed':
						...
						{
							case '1' :	// not active
								MP_Users::set_status($mp_user->id, 'waiting');
							break;
							case '2' :	// tbc
								MP_Users::set_status($mp_user->id, 'waiting');
								MP_Users::send_confirmation_subscription($mp_user->email, $mp_user->name, $mp_user->confkey);
			...
			else
			{
				...
				{
					case '1' :	// not active
						MP_Users::insert($visitor_toemail, $visitor_toname);
					break;
					case '2' :	// tbc
		...
		$mail = new stdClass();
		$mail->id = MP_Mails::get_id('MP_Forms::process2');

		// duplicate attachements
		if (!empty($attachements))
		{
			$metas = MP_Mailmeta::has( $mail_main_id, '_MailPress_attached_file');
			if ($metas)
			{
...
//// Building form
	...
	public static function get_field($field, $html = false, $form_template = false)
	{
		...
		$search[] = '{{field_id}}'; 	$replace[] = MP_Forms_field_types::get_id($field);
		$search[] = '{{field}}'; 	$replace[] = MP_Forms_field_types::get_tag($field);
		$html = str_replace($search, $replace, $html ); 
		return $html;
	}

	public static function handle_map($source) 
	{
		$dest = 'tmp/MP_Forms_' . md5(uniqid(rand(), 1)) . '.gif';
		if (copy($source, MP_ABSPATH . $dest)) { fclose(fopen(MP_ABSPATH . $dest, 'r+')); return $dest;}
		return false;
	}
