...
//// Html ////
	...
	public static function get_url_parms($parms = array('mode', 'status', 's', 'apage', 'author', 'startwith'))
	{
		...
		{
			if (isset($_GET[$parm]))  $url_parms[$parm] = trim(stripslashes($_GET[$parm]));
			if (isset($_POST[$parm])) $url_parms[$parm] = trim(stripslashes($_POST[$parm]));
		}
		if ((isset($url_parms['s'])) && empty($url_parms['s']) )		unset($url_parms['s']);
		...
		if (!isset($url_parms['mode']  )) 						$url_parms['mode'] = 'list';
		return $url_parms;
	}
